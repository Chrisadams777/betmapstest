...
?>
	...
	<script>
        ...
        function sendAuthorImage(path, action) {
            ...
            if ($('#article input[type="file"][name="author_image-image"]').lenght &&
        ...
        function sendCoverImage(form_id, path, acc_id, action) {
            ...
            if($("#"+form_id+' input[type="file"][name="cover-image"]').lenght &&
    ...
    <?php if (string_strpos($_SERVER["PHP_SELF"], "/".LISTING_FEATURE_FOLDER."/") !== false || string_strpos($_SERVER["PHP_SELF"], "/claim/") !== false) { ?>
        ...
        <script type="text/template" id="templateFeature">
            <div class="group-feature" data-feature-id="%item_id%">
                ...
                <input type="hidden" name="features[%item_id%][title]" class="input-feature-title" value="%item_title%">
        ...
        <script type="text/javascript">
            $(document).on('click', '.btn-add-hours', function (e) {
                ...
                var hours_start = $(".hours-start option:selected").text();
                var hours_end = $(".hours-end option:selected").text();

                var weekday_value = $(".weekday option:selected").val();
                var hours_start_value = $(".hours-start option:selected").val();
                var hours_end_value = $(".hours-end option:selected").val();

                if(weekday_value && hours_start_value && hours_end_value){
                    if(hours_start_value < hours_end_value){
                        ...
                        if ($('.hours-start')[0].selectize){
                            $('.hours-start')[0].selectize.clear();
                        }
                        if ($('.hours-end')[0].selectize){
                            $('.hours-end')[0].selectize.clear();
                        }
                    } else {
                        $(".alert-hours-of-work").html('<?=system_showText(LANG_HOURS_WORK_START_LESS_END);?>').fadeIn();
                    }
                }
            ...
            $(document).on('click', '.btn-save-feature', function (e) {
                ...
                if(item_icon !== '' && item_title !== ''){
                    if (item_id === '') {
                        ...
                        $group_feature.find('.input-feature-title').val(item_title);
